...
if ($_FILES['img_2']['size'] < 2000000){
    ...
    if ($exten=='jpg' || $exten=='jpeg'){
		...
		if ($check > 0){
			...
			$update = "update project_teacher_support_training_master set image_2='".$dst."' where project_no='".$_REQUEST['prno']."'";

			mysql_query($update);
		}else{

			$insert = "insert into project_teacher_support_training_master(project_no, image_2) values('".$_REQUEST['prno']."', '".$dst."')";
