...
if ($_FILES['img_5']['size'] < 2000000){

	$extension = explode('.', $_FILES["img_5"]["name"]);
    $exten = $extension[1];
    ...
    if ($exten=='jpg' || $exten=='jpeg'){
		...
		$fnm = $_FILES['img_5']['name'];
		$dst = 'images/'.$folder.'/'.$fnm;

		$upload = move_uploaded_file($_FILES['img_5']['tmp_name'], $dst);
		...
		if ($check > 0){
			...
			$update = "update project_temp_adult_literacy set image_5='".$dst."' where project_no='".$_REQUEST['projectId']."' and sln='".$_REQUEST['sln']."'";

			mysql_query($update);
		}else{

			$insert = "insert into project_temp_adult_literacy(project_no, image_5, sln) values('".$_REQUEST['projectId']."', '".$dst."', '".$_REQUEST['sln']."')";
