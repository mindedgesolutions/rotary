...
function deleteArticle(par){
  ...
  if (confirm("Sure you wish to delete this video?")){
...
<div class="wrapper">
 ...
 <?php include('include/header.php'); ?>
    ...
    <?php include 'pagination_video.php'; ?>

    <section class="content-header">
      <ol class="breadcrumb">
        <li><a href="index.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Add/Edit Images</li>
      </ol>
    </section>
  <div class="container">
    <div class="row">
    ...
    <div class="col-sm-12">
    <a href="generate-gallery-id.php"><input type="button" name="addBtn" id="addBtn" class="btn btn-primary" value="Add Image" /></a>
    </div>
      ...
      <div class="col-sm-12">
        ...
        <tr class="table-tr" style="background-color:#1980cf;color:#ffffff;">
          <td class="col-sm-2">Published On</td>
          <td class="col-sm-3">Video Header</td>
          <td class="col-sm-4">Video Details</td>
          <td class="col-sm-1">Status</td>
          <td class="col-sm-2">Action</td>
        </tr>

        <?php
        $checkRow = mysql_num_rows(mysql_query("select * from video_master where video_id!=''"));

        if ($checkRow > 0){

          $sln=1;

          $query_getDet = mysql_query("select * from video_master where video_id!='' order by SLN desc $limit");
          while ($getDet = mysql_fetch_array($query_getDet)){

            $ytCode = substr($getDet['video'], -11);

            $date = explode('/', $getDet['video_id']);
            $getDate = $date[1];
          ...
          <tr class="table-tr">
            ...
            <td class="col-sm-3" style="line-height: 20px;"><?= strtoupper(urldecode($getDet['video_header'])) ?></td>

            <td class="col-sm-4" style="padding: 8px 0;">
              <!--<video width='270' height='202.5' controls><source src="<?php echo $getDet['video'] ?>" type='video/mp4'></source></video>-->

              <?php if ($getDet['video']!=''){ ?>

              <iframe width="267" height="150" src="https://www.youtube.com/embed/<?= $ytCode ?>" frameborder="0" allowfullscreen></iframe>

              <?php }else{echo 'Video is not available';} ?>
            </td>

            <td class="col-sm-1"><?= $status ?></td>

            <td class="col-sm-2">
              <a href="add-edit-video.php?vid=<?= base64_encode($getDet['video_id']) ?>"><input type="button" name="editBtn" id="editBtn" class="btn btn-primary" value="Edit" style="width: 60px;margin: 0 10px 0 0;" /></a>
      ...
      <?php
        $rowNum = mysql_fetch_array(mysql_query("select count(SLN) as rowCount from video_master where video_id!=''"));
