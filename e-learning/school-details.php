...
<?php
$getDet = mysql_fetch_array(mysql_query("select * from project_temp_elearning where project_no='".$_REQUEST['prno']."' and sln='".$_REQUEST['sln']."'"));
?>
...
$(document).ready(function(){
	...
	$(document).on('change', '#requirement_letter', function(){
		...
		if (jQuery.inArray(image_extension, ['pdf', 'docx', 'jpg', 'jpeg']) == -1){
...
function saveData(){
	...
	var requirement_letter = $('#requirement_letter').val();
	var satisfaction_letter = $('#satisfaction_letter').val();
	...
	}else if (requirement_letter==''){

		alert('Please upload letter of requirement');
		document.getElementById('requirement_letter').focus();

	}else if (satisfaction_letter==''){

		alert('Please upload letter of satisfaction');
		document.getElementById('satisfaction_letter').focus();
	}else{
