...
if ($_FILES['photo_2']['size'] < 2000000){

	$extension = explode('.', $_FILES["photo_2"]["name"]);
    $exten = $extension[1];
    ...
    if ($exten=='jpg' || $exten=='jpeg' || $exten=='png'){
		...
		$fnm = $_FILES['photo_2']['name'];
		$dst = 'images/'.$folder.'/'.$fnm;

		$upload = move_uploaded_file($_FILES['photo_2']['tmp_name'], $dst);
		...
		if ($check > 0){
			...
			$update = "update project_temp_elearning set image_2='".$dst."' where project_no='".$_REQUEST['projectId']."' and sln='".$_REQUEST['sln']."'";

			mysql_query($update);
		}else{

			$insert = "insert into project_temp_elearning(project_no, image_2, sln) values('".$_REQUEST['projectId']."', '".$dst."', '".$_REQUEST['sln']."')";
