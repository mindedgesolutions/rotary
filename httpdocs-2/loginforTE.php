...
if(isset($_POST["btn_submit"])) {
	...
	$pwd =  $_POST["txtpwd"];
	$msg = "";
	$loginqry = "SELECT count(id) as avail,id FROM teacherevaluationuser WHERE username='".$username."' and password='".$pwd."';";
	$result = mysqli_query($link,$loginqry);
		...
		if($row[0]["avail"]>0) {
			...
			$checkqry = "SELECT count(id) as totalschool from schoolInfo where userid=".$row[0]["id"]." ;";
			$chkres = mysqli_query($link,$checkqry);
			$chkrow[]= mysqli_fetch_assoc($chkres);
			if($chkrow[0]["totalschool"]>0) {
			header("Location: teacherEvaluationForm.php");
			}
			else
			{
			header("Location: evaluateschool.php");
			}
		}
                        ...
                        <tr>
                            <td width="650" valign="top">
                                <table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" style="margin-bottom:5px">
                                    <tr>
                                        <td align="left" colspan="3">
                                       	  <h1 style="padding:0; margin:0">Login - Teacher Evaluation Form</h1>
                                      </td>
                                ...
                                <div style="margin:15px 0 10px 0; line-height:18px; color:#CC0000; " class="auto-style1">
									<strong>
									<?php echo $msg; ?>
									Kindly <a href="te.htm" target="_blank">
									CLICK HERE</a> to read the instructions 
									carefully before Registering</strong></div>
    ...
    <tr>
        <td>
            <p style="padding:15px 15px 0 15px; margin:0">
                <form id="TELogin" name="TELogin" action="loginforTE.php" method="post" onsubmit="return validation()">
                ...
                <tr>
                    ...
                    <td>
                        ...
                        <input type="reset" name="btn_reset" id="btn_reset" value="Cancel"  class="hed" style="cursor:pointer; border-radius:3px" />&nbsp;&nbsp;<a href="registerationforTE.php">New User / Register</a>
