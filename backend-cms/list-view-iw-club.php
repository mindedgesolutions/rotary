...
<div class="wrapper">
 ...
 <?php include('include/header.php'); ?>
    ...
    <?php include 'pagination_iw_club.php'; ?>

    <section class="content-header">
      <ol class="breadcrumb">
        <li><a href="index.php"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">List View - InnerWheel Clubs</li>
      </ol>
    </section>
  <div class="container">
    <div class="row">
    ...
    <a href="add-edit-club.php?cname="><input type="button" name="addBtn" id="addBtn" class="btn btn-primary" value="Add New Club" /></a>
      ...
      <table class="col-sm-12" border="1" style="border-collapse: collapse;text-align: center;">
        ...
        <?php
        $checkRow = mysql_num_rows(mysql_query("select * from iwdistrictclub"));

        if ($checkRow > 0){

          $query_getDet = mysql_query("select * from iwdistrictclub order by id asc $limit");
          while ($getDet = mysql_fetch_array($query_getDet)){

          ?>
          <tr class="table-tr">
            <td class="col-sm-2"><?= $getDet['id'] ?></td>

            <td class="col-sm-5"><?= strtoupper($getDet['IWclub']) ?></td>

            <td class="col-sm-2"><?= $getDet['IWdistrict'] ?></td>

            <td class="col-sm-1"><?= strtoupper($getDet['category']) ?></td>

            <td class="col-sm-2"><a href="add-edit-club.php?cname=<?= base64_encode($getDet['IWclub']) ?>"><input type="button" name="editBtn" id="editBtn" class="btn btn-primary" value="Edit Details" /></a></td>
          </tr>
          <?php }}else{ ?>
          <tr class="table-tr">
      ...
      <?php
        $rowNum = mysql_fetch_array(mysql_query("select count(id) as rowCount from iwdistrictclub"));
