...
function saveData(){
	...
	var teacher_name_1 = $('#teacher_name_1').val();
	var no_student_1 = $('#no_student_1').val();
	var teacher_name_2 = $('#teacher_name_2').val();
	var no_student_2 = $('#no_student_2').val();
	var additional_teacher = $('#additional_teacher').val();
	var additional_students = $('#additional_students').val();
	...
	if (schoolId==''){
	...
	}else if (teacher_name_1==''){

		alert('Please enter name of teacher - 1');
		document.getElementById('teacher_name_1').focus();
	}else if (no_student_1==''){

		alert('Please enter no. of students');
		document.getElementById('no_student_1').focus();
	}else if (teacher_name_2==''){

		alert('Please enter name of teacher - 2');
		document.getElementById('teacher_name_2').focus();
	}else if (no_student_2==''){

		alert('Please enter no. of students');
		document.getElementById('no_student_2').focus();
	}else{
		...
		$.ajax({
			...
			type: 'post',
			data: 'sln=' + sln + '&projectId=' + projectId + '&schoolId=' + schoolId + '&schoolName=' + schoolName + '&schoolCity=' + schoolCity + '&schoolState=' + schoolState + '&schoolType=' + schoolType + '&no_schools=' + no_schools + '&trainingType=' + trainingType + '&teacher_name_1=' + teacher_name_1 + '&no_student_1=' + no_student_1 + '&teacher_name_2=' + teacher_name_2 + '&no_student_2=' + no_student_2 + '&additional_teacher=' + additional_teacher + '&additional_students=' + additional_students,
			success: function(f){
...
<?php
$getTemp = mysql_fetch_array(mysql_query("select * from project_temp_teacher_support_training where project_no='".$_REQUEST['prno']."' and sln='".$_REQUEST['sln']."'"));
?>
...
<table width="500" border="0" style="border-collapse: collapse;margin: 5px 0 0 0;">
	...
	<tr>
		...
		<td width="55%" class="table-row">
			<input type="text" name="teacher_name_1" id="teacher_name_1" value="<?php echo $getTemp['teacher_name_1'] ?>" class="form-control" style="width: 95%;" onkeyup="letterswithspace(this)" />
		</td>
	</tr>
	...
	<tr>
		...
		<td width="55%" class="table-row">
			<input type="text" name="no_student_1" id="no_student_1" value="<?php if ($getTemp['student_no_1']!=''){echo $getTemp['student_no_1'];} ?>" class="form-control" style="width: 95%;" onkeyup="numbersOnly(this)" />
		</td>
	</tr>
	...
	<tr>
		...
		<td width="55%" class="table-row">
			<input type="text" name="teacher_name_2" id="teacher_name_2" value="<?php echo $getTemp['teacher_name_2'] ?>" class="form-control" style="width: 95%;" onkeyup="letterswithspace(this)" />
		</td>
	</tr>
	...
	<tr>
		<td width="45%" class="table-row">No. of students taught : </td>
		<td width="55%" class="table-row">
			<input type="text" name="no_student_2" id="no_student_2" value="<?php if ($getTemp['student_no_2']!=''){echo $getTemp['student_no_2'];} ?>" class="form-control" style="width: 95%;" onkeyup="numbersOnly(this)" />
		</td>
	</tr>

	<tr>
		<td width="100%" class="table-row" colspan="2" style="line-height: 25px;">If more than 2 teachers involved, please provide the total number of teachers trained <input type="text" name="additional_teacher" id="additional_teacher" value="<?php echo $getTemp['total_teacher'] ?>" class="form-control" style="width: 50px;height: 25px;font-size: 12px;text-align: center;" onkeyup="numbersOnly(this)" /> and total number of students taught by all the teachers trained <input type="text" name="additional_students" id="additional_students" value="<?php echo $getTemp['total_student'] ?>" class="form-control" style="width: 50px;height: 25px;font-size: 12px;text-align: center;" onkeyup="numbersOnly(this)" />. </td>
	</tr>
