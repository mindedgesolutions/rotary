...
function deleteArticle(par){
  ...
  if (confirm("Sure you wish to delete this image?")){

    $.ajax({

      url: 'delete-teacher-support.php',
      type: 'post',
      data: 'dbsln=' + dbsln,
      success: function(f){
...
<div class="wrapper">
 ...
 <?php include('include/header.php'); ?>
    ...
    <?php include 'pagination_childdevelopment.php'; ?>

    <section class="content-header">
      <ol class="breadcrumb">
        <li><a href="index.php"><i class="fa fa-dashboard"></i>Home</a></li>
        <li class="active">Add/Edit Child Development</li>
      </ol>
    </section>
  <div class="container">
    <div class="row">
    ...
    <div class="col-sm-12">
    <a href="generate-childdevelopment.php"><input type="button" name="addBtn" id="addBtn" class="btn btn-primary" value="Add Image" /></a>
    </div>
      ...
      <div class="col-sm-12">
        ...
        <tr class="table-tr" style="background-color:#1980cf;color:#ffffff;">
          ...
          <td class="col-sm-3">Image Header</td>
          <td class="col-sm-4">Image Details</td>
          <td class="col-sm-1">Status</td>
          <td class="col-sm-2">Action</td>
        </tr>

        <?php
        $checkRow = mysql_num_rows(mysql_query("select * from image_master where image_id!='' and gallery_type='Child Development'"));

        if ($checkRow > 0){

          $sln=1;

          $query_getDet = mysql_query("select * from image_master where image_id!='' and gallery_type='Child Development' order by SLN desc $limit");
          while ($getDet = mysql_fetch_array($query_getDet)){

            $date = explode('/', $getDet['image_id']);
            $getDate = $date[1];
          ...
          <tr class="table-tr">
            ...
            <td class="col-sm-3" style="line-height: 20px;"><?= urldecode($getDet['image_header']) ?></td>

            <td class="col-sm-4" style="padding: 8px 0;">
              <img src="<?= $getDet['image'] ?>" style="height: 80px;">
            </td>

            <td class="col-sm-1"><?= $status ?></td>

            <td class="col-sm-2">
              <a href="add-edit-childdevelopment.php?imgid=<?= base64_encode($getDet['image_id']) ?>"><input type="button" name="editBtn" id="editBtn" class="btn btn-primary" value="Edit" style="width: 60px;margin: 0 10px 0 0;" /></a>
      ...
      <?php
        $rowNum = mysql_fetch_array(mysql_query("select count(SLN) as rowCount from image_master where image_id!='' and gallery_type='Child Development'"));
