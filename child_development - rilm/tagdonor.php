...
$sqlNoTag="select donar_id as donaridnotag,FLOOR(actual_tagging) amount_no_child from tbl_donar_master where tagged='no' and voucherNo='ND2010'";
/*select donar_id as donaridnotag,FLOOR(amount_no_child) as  amount_no_child from tbl_donar_master where  donar_id in (9548,9514,9489,10722)";*/
/*$sqlNoTag="select donor_id as donaridnotag,amount_no_child as  amount_no_child from tmp_positive_discrepancy";*/
 ...
 while($rowchilNoTag = mysql_fetch_array($resultNoTag)){
	   ...
	   extract($rowchild);
		...
		if ($childNo > $totchild)
		{
		   ...
		   for ($x = 0; $x < $nochild; $x++) {
			   $childUpdate = "select child_id as child_id_upd from tbl_child_profile_card where tagged ='no' limit 1";
			   ...
			   extract($rowchildupd);
					...
					$sqlupdate = "insert into tbl_child_selected_tagging (child_id,donar_id,date_of,time_of) values ('$child_id_upd','$id',CURDATE(),CURTIME())";
					  ...
					  $sqlupdate = "update tbl_donar_master set amount_no_child=FLOOR(amount_no_child) + 1 where donar_id='$id'";
					  $resultchild3 = mysql_query($sqlupdate);
				}
			}
		}
		/*else
