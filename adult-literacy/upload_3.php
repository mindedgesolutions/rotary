...
if ($_FILES['img_3']['size'] < 2000000){
    ...
    if ($exten=='jpg' || $exten=='jpeg'){
		...
		if ($check > 0){
			...
			$update = "update project_temp_adult_literacy set image_3='".$dst."' where project_no='".$_REQUEST['projectId']."' and sln='".$_REQUEST['sln']."'";

			mysql_query($update);
		}else{

			$insert = "insert into project_temp_adult_literacy(project_no, image_3, sln) values('".$_REQUEST['projectId']."', '".$dst."', '".$_REQUEST['sln']."')";
