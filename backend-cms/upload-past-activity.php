...
<div class="wrapper">
 ...
 <?php include('include/header.php'); ?>
    ...
    <?php include 'pagination_past.php'; ?>
  ...
  <div class="container">
    <div class="row">
		...
		<div class="col-sm-12">
		<a href="generate-past-id.php"><input type="button" name="addBtn" id="addBtn" class="btn btn-primary" value="Add Activity" /></a>
		</div>
      ...
      <table class="col-sm-12" border="1" style="border-collapse: collapse;text-align: center;">
        ...
        <?php
        $checkRow = mysql_num_rows(mysql_query("select * from past_act where article_id!=''"));

        if ($checkRow > 0){

          $sln=1;

          $query_getDet = mysql_query("select * from past_act where article_id!='' order by SLN desc $limit");
          ...
          <tr class="table-tr">
            ...
            <td class="col-sm-5"><?= urldecode($getDet['article_header']) ?></td>
            ...
            <td class="col-sm-2">
              <a href="add-edit-past.php?aid=<?= base64_encode($getDet['article_id']) ?>"><input type="button" name="editBtn" id="editBtn" class="btn btn-primary" value="Edit" style="width: 60px;margin: 0 10px 0 0;" /></a>
      ...
      <?php
        $rowNum = mysql_fetch_array(mysql_query("select count(SLN) as rowCount from past_act where article_id!=''"));
