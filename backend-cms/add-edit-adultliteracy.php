 ...
 <?php include('include/header.php'); ?>
    ...
    <section class="content-header">
      <ol class="breadcrumb">
        ...
        <li class="active">Add/Edit Adult Literacy Images</li>
      </ol>
    </section>
    ...
    if (isset($_REQUEST['addBtn'])){
      ...
      if (!empty($_FILES['product_img']['name'])){
        ...
        if ($exten=='jpg' || $exten=='JPG' || $exten=='JPEG' || $exten=='png'){
          ...
          $dst = 'gallery-images/adult-literacy/'.$fnm;

          $upload = move_uploaded_file($_FILES['product_img']['tmp_name'], $dst);

          $update = "update image_master set image='".$dst."', image_name='".urlencode($_REQUEST['image_name'])."', image_header='".urlencode($_REQUEST['image_header'])."', gallery_type='Adult Literacy', status='".$_REQUEST['status']."' where image_id='".base64_decode($_REQUEST['imgid'])."'";

          mysql_query($update);
        ?>
        <script type="text/javascript">
          alert('Image has been uploaded');
          window.location = 'upload-adultliteracy-gallery.php';
        </script>
        <?php
        }else{
        ?>
        ...
        $update = "update image_master set image_name='".urlencode($_REQUEST['image_name'])."', image_header='".urlencode($_REQUEST['image_header'])."', gallery_type='Adult Literacy', status='".$_REQUEST['status']."' where image_id='".base64_decode($_REQUEST['imgid'])."'";
      ...
      <table width="100%">
        ...
        <tr class="table-tr">
          <td class="col-sm-12" colspan="2" style="text-align: center;">
            ...
            <a href="upload-adultliteracy-gallery.php"><input type="button" name="backBtn" id="backBtn" class="btn btn-primary" value="Back to List" style="margin: 0 15px 0 0;" /></a>
