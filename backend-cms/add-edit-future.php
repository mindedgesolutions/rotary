...
<?php
$getDet = mysql_fetch_array(mysql_query("select * from future_act where article_id='".base64_decode($_REQUEST['aid'])."'"));
?>
    ...
    if (isset($_REQUEST['addBtn'])){
      ...
      if (!empty($_FILES['product_img']['name'])){
        ...
        if ($exten=='jpg' || $exten=='JPG' || $exten=='JPEG' || $exten=='png'){
          ...
          $dst = 'img_future_act/'.$fnm;

          $upload = move_uploaded_file($_FILES['product_img']['tmp_name'], $dst);

          $update = "update future_act set image='".$dst."', article_header='".urlencode($_REQUEST['article_header'])."', article_content='".urlencode($_REQUEST['article_content'])."', image_name='".urlencode($_REQUEST['image_name'])."', status='".$_REQUEST['status']."', link_text='".urlencode($_REQUEST['link_text'])."', link_url='".urlencode($_REQUEST['link_url'])."' where article_id='".base64_decode($_REQUEST['aid'])."'";

          mysql_query($update);
        ?>
        <script type="text/javascript">
          alert('Article has been updated');
          window.location = 'upload-future-activity.php';
        </script>
        <?php
        }else{
        ?>
        ...
        $update = "update future_act set article_header='".urlencode($_REQUEST['article_header'])."', article_content='".urlencode($_REQUEST['article_content'])."', image_name='".urlencode($_REQUEST['image_name'])."', status='".$_REQUEST['status']."', link_text='".urlencode($_REQUEST['link_text'])."', link_url='".urlencode($_REQUEST['link_url'])."' where article_id='".base64_decode($_REQUEST['aid'])."'";
      ...
      <table width="100%">
        ...
        <tr class="table-tr">
          <td class="col-sm-12" colspan="2" style="text-align: center;">
            ...
            <a href="upload-future-activity.php"><input type="button" name="backBtn" id="backBtn" class="btn btn-primary" value="Back to List" style="margin: 0 15px 0 0;" /></a>
