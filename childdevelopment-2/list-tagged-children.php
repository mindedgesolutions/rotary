...
while ($getDet = mysql_fetch_array($query_getDet)){
	...
	$donorDet = mysql_fetch_array(mysql_query("select first_name, last_name from tbl_donar_master where donar_id='".$getDet['donar_id']."'"));
	...
	$dname = $donorDet['first_name'].' '.$donorDet['last_name'];
	$donorName = str_replace(",", "+", $dname);

	$msg=$msg.$getDet['child_id'].",".$childname.",".$fathername.",".$mothername.",".$state.",".$ngo.",".$getDet['donar_id'].",".$donorName."\r\n";
		$i++;
}
