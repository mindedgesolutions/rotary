 ...
 <?php include('include/header.php'); ?>
    ...
    <section class="content-header">
      <ol class="breadcrumb">
        ...
        <li class="active">Add/Edit Teacher Support Images</li>
      </ol>
    </section>
    ...
    if (isset($_REQUEST['addBtn'])){
      ...
      if (!empty($_FILES['product_img']['name'])){
        ...
        if ($exten=='jpg' || $exten=='JPG' || $exten=='JPEG' || $exten=='png'){
          ...
          if ($_REQUEST['gallery_subtype']=='Teacher Support'){

            $fnm = $_FILES['product_img']['name'];
            $dst = 'gallery-images/teacher-support/teacher-support/'.$fnm;
          }else{

            $fnm = $_FILES['product_img']['name'];
            $dst = 'gallery-images/teacher-support/nation-builder/'.$fnm;
          }

          $upload = move_uploaded_file($_FILES['product_img']['tmp_name'], $dst);

          $update = "update image_master set image='".$dst."', image_name='".urlencode($_REQUEST['image_name'])."', image_header='".urlencode($_REQUEST['image_header'])."', gallery_type='Teacher Support', status='".$_REQUEST['status']."', gallery_subtype='".$_REQUEST['gallery_subtype']."' where image_id='".base64_decode($_REQUEST['imgid'])."'";

          mysql_query($update);
        ?>
        <script type="text/javascript">
          alert('Image has been uploaded');
          window.location = 'upload-teacher-gallery.php';
        </script>
        <?php
        }else{
        ?>
        ...
        $update = "update image_master set image_name='".urlencode($_REQUEST['image_name'])."', image_header='".urlencode($_REQUEST['image_header'])."', gallery_type='Teacher Support', status='".$_REQUEST['status']."', gallery_subtype='".$_REQUEST['gallery_subtype']."' where image_id='".base64_decode($_REQUEST['imgid'])."'";
  ...
  <div class="container">
    <div class="row">
      ...
      <table width="100%">
        ...
        <tr class="table-tr">
          <td class="col-sm-4">Select Gallery Type : </td>
          <td class="col-sm-8">

            <select class="form-control" name="gallery_subtype" id="gallery_subtype">

              <option value="Teacher Support" <?php if ($getDet['gallery_subtype']=='Teacher Support'){echo 'selected';} ?>>Teacher Support</option>

              <option value="Nation Builder Award" <?php if ($getDet['gallery_subtype']=='Nation Builder Award'){echo 'selected';} ?>>Nation Builder Award</option>
        ...
        <tr class="table-tr">
          <td class="col-sm-12" colspan="2" style="text-align: center;">
            ...
            <a href="upload-teacher-gallery.php"><input type="button" name="backBtn" id="backBtn" class="btn btn-primary" value="Back to List" style="margin: 0 15px 0 0;" /></a>
